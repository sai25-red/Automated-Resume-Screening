...
def UserImageTest(request):
    ...
    if request.method == 'POST':
        ...
        uploaded_file_path = os.path.join(settings.MEDIA_ROOT, 'test_pdf', filename)
        print(uploaded_file_path)

        from .utility import predict

        result = predict.pdf(uploaded_file_path)
        print(result)
       
        result = f'Congrats your resume is  for a {result} profile'
        print('result', result)
        return render(request, "users/test_form.html", {"path":uploaded_file_path,"result": result})
    else:
